...
# Phase-1 default org UUID (single-org mode).  Replace with real JWT extraction later.
...
async def _get_default_org_id(db: AsyncSession) -> uuid.UUID:
    """Return the first active org, or the hardcoded default UUID in single-org mode."""
    try:
        result = await db.execute(
            select(Organization.id).where(Organization.is_active).limit(1)
        )
