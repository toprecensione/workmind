...
class Organization(Base):
    ...
    watch_paths: Mapped[list["KbWatchPath"]] = relationship(
        "KbWatchPath", back_populates="organization", lazy="raise"
    )
...
class User(Base):
    ...
    totp_secret: Mapped[Optional[str]] = mapped_column(String(64))  # TOTP base32 secret
...
class Conversation(Base):
    ...
    channel_type: Mapped[str] = mapped_column(
        String(32), nullable=False, default=ChannelType.web.value
    )
...
class Message(Base):
    ...
    provider: Mapped[Optional[AIProvider]] = mapped_column(
        Enum(AIProvider, name="ai_provider", native_enum=False, length=32)
    )
    model_name: Mapped[Optional[str]] = mapped_column(String(128))
    model_role: Mapped[Optional[ModelRoleEnum]] = mapped_column(
        Enum(ModelRoleEnum, name="model_role", native_enum=False, length=16)
    )
...
class KbWatchPath(Base):
    ...
    organization: Mapped["Organization"] = relationship("Organization", back_populates="watch_paths")
...
class ModelUsage(Base):
    ...
    provider: Mapped[AIProvider] = mapped_column(
        Enum(AIProvider, name="ai_provider", native_enum=False, length=32), nullable=False
    )
...
class Job(Base):
    ...
    status: Mapped[JobStatus] = mapped_column(
        ...
        nullable=False, default=JobStatus.pending
    )
...
# ── Client model re-exports (backward compat) ───────────────────────
# MEDIC models live in clients/medic/models.py
# Re-exported here so existing imports keep working unchanged.
from clients.medic.models import (  # noqa: E402, F401
    MedicProduct,
    MedicInventoryLot,
    MedicSale,
    MedicStockCorrection,
    AgentWarehouseAllocation,
    AgentWarehouseMovement,
)
