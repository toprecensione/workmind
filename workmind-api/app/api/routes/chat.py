...
async def chat(
    ...
    # ── Call AI via ModelRouter (non-streaming) ────────────────────────────────
    ...
    # ── Record model usage ─────────────────────────────────────────────────────
    from app.db.models import ModelUsage  # noqa: PLC0415
...
@router.post(
    "/chat/stream",
    summary="Streaming chat — returns text/event-stream (SSE)",
...
async def chat_stream(
    ...
    """
    SSE streaming chat. Yields:
      data: {"type":"chunk","text":"..."}      — incremental text
      data: {"type":"done","conversation_id":"...","message_id":"...","provider":"...","model":"..."}
      data: {"type":"error","message":"..."}   — on failure

    CSRF note: JWT in Authorization header (not cookie) — inherently CSRF-safe.
    Nginx buffering must be disabled: X-Accel-Buffering: no header is set.
    """
    org_id = current_user.org_id
    user_id = current_user.user_id
    ...
    if settings.anonymize_before_external_ai:
        try:
            anon_result = await anonymizer.anonymize(body.message, session_key=session_key)
            anon_content = anon_result.anonymized_text
        except Exception:
    ...
    primary, _, model_name = router_svc._resolve_route(model_role)
    ...
    async def _sse_generator() -> AsyncGenerator[str, None]:
        ...
        try:
            async for chunk in router_svc.complete_stream(
                messages=history,
                current_message=anon_content,
                role=model_role,
                org_id=org_id,
            ...
            yield f"data: {json.dumps({'type': 'error', 'message': 'Errore nella generazione. Riprova.'})}\n\n"
            return

        # ── De-anonymize and persist ───────────────────────────────────────────
        reply_text = full_text
        if settings.anonymize_before_external_ai and anon_content != body.message:
            try:
                reply_text = await anonymizer.deanonymize(full_text, session_key=session_key)
            except Exception:
                pass

        latency_ms = int((time.monotonic() - t_start) * 1000)

        # Rough token estimate (2 tokens per word — conservative)
        est_input_tokens = max(len(anon_content.split()) * 2, 1)
        est_output_tokens = max(len(full_text.split()) * 2, 1)

        from app.db.models import AIProvider, ModelRoleEnum

        # Map provider string to AIProvider enum safely
        try:
            ai_provider = AIProvider(primary)
        ...
        assistant_msg = Message(
            ...
            role=MessageRole.assistant,
            content=reply_text,
            content_anon=full_text if full_text != reply_text else None,
        ...
        try:
            ...
            log.error("chat_stream_db_commit_failed", error=str(exc))
        ...
        yield f"data: {json.dumps({'type': 'done', 'conversation_id': str(conversation.id), 'message_id': str(assistant_msg.id), 'provider': primary, 'model': model_name, 'latency_ms': latency_ms})}\n\n"
