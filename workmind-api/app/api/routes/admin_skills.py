...
async def list_skills(
    ...
    for sid, sdef in SKILL_CATALOG.items():
        ...
        out.append(SkillOut(
            ...
            allowed_roles=sk.allowed_roles if sk else None,
        ))
    return out
...
@router.patch("/skills/{skill_id}/roles")
async def set_skill_roles(
    ...
    """Imposta i ruoli che possono usare questa skill. None = tutti."""
    _require_admin(user)
    if skill_id not in SKILL_CATALOG:
        raise HTTPException(404, f"Skill '{skill_id}' non esiste")
    org_id = str(user.org_id)
    ...
    if row:
        ...
        db.add(Skill(id=uuid4(), org_id=org_id, skill_id=skill_id, is_enabled=False,
                     config_json={}, allowed_roles=body.allowed_roles or None))
    await db.commit()
    return {"ok": True}
